...
def save_results(input_img, gt_data,density_map,output_dir, fname='results.png'):
    ...
    # display the resolution of input image
...
def cv2_blend(foreground, background, alpha):
    img1 = foreground * alpha
    img2 = background * (1 - alpha)
    result_img = cv2.add(img1, img2)
    return result_img
    
def save_blend_results(input_img, gt_data,density_map,output_dir, fname='results.png', alpha = 0.5):
    input_img = input_img[0][0]
    ...
    # display the resolution of input image
    #print("input image size: ", input_img.shape)
    #print("density map size:", density_map.shape)
    
    # resize density map to match the shape of input image
    density_map = cv2.resize(density_map, (input_img.shape[1],input_img.shape[0]))
    
    result_img = cv2_blend(density_map, input_img, alpha)
...
def save_density_map(density_map,output_dir, fname='results.png'):    
    ...
    # display the resolution of predicted density image
    #print("density map size: ", density_map.shape)
...
def display_results(input_img, gt_data,density_map):
    ...
    # display the resolution of input image
    #print("imput image size: ", imput_img.shape)
